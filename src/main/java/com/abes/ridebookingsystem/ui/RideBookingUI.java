...
public class RideBookingUI {
	...
	private void bookRide(Customer customer) {
		...
		// Fare Calculation
		int base = selectedVehicle.getBaseFare();
		int perKm = selectedVehicle.getPerKmRate();
		double totalFare = base + (distance * perKm);
		...
		while (true) {
			...
			String paymentStr = scanner.nextLine().trim().toUpperCase();
			if (ValidationUtil.isValidEnum(PaymentMethod.class, paymentStr)) {
